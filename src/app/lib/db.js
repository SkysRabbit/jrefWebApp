...
configureDatabase().catch(
   error => console.log('db config err', error)
)
...
export async function getMinLinks(limit, offset) {
   ...
   }).from(LinksTable).limit(lookupLimit).offset(lookupOffset).orderBy
   (desc(LinksTable.createdAt)).where(eq(LinksTable.userId, sessionUser));
}
...
export async function getMinLinksAndVisits(limit, offset) {
   ...
   //       visitCount: sqld`count(${VisitsTable.id})`.as('count')
   //    })
   //    .from(LinksTable)
   //    .leftJoin(VisitsTable, eq(VisitsTable.linkId, LinksTable.id))
